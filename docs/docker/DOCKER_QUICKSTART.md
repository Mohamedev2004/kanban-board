...
### Windows (PowerShell)
```powershell
.\scripts/docker-start.ps1    # Démarrer
.\scripts/docker-stop.ps1     # Arrêter
.\scripts/docker-logs.ps1     # Voir logs
.\scripts/docker-status.ps1   # État
```

### Linux / macOS
```bash
./scripts/docker-start.sh     # Démarrer
./scripts/docker-stop.sh      # Arrêter
./scripts/docker-logs.sh      # Voir logs
./scripts/docker-status.sh    # État
```
